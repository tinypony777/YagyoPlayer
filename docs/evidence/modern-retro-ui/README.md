...
- 円形波形の `静 / 響 / 烈` は絶対音量帯から分離し、曲中の局所floor/ceilingへ相対化した。圧縮音源の定常部は `響`へ戻り、小さな相対差を表示へ残す。サビ・セクション・規格ラウドネスの検出ではない。
...
- Xcode 27.0 generic iOS build: PASS
  - `review-relative-generic-20260713/generic.xcresult`
- iPhone 17 Pro / iOS 26.5 full suite: **127 / 127 PASS、failure 0、skip 0**
  - `review-relative-full-ios26-20260713/full.xcresult`
- iPhone 17 Pro / iOS 27.0 UI・解析・モデル等（PlaybackController群を除く）: **115 / 115 PASS、failure 0、skip 0**
  - `review-relative-ios27-no-playback-20260713/no-playback.xcresult`
- iPhone 17 Pro / iOS 26.5 PlaybackController: **12 / 12 PASS、failure 0、skip 0**
  - 127件のfull suiteに包含。
- 圧縮音源を模した円形波形の相対表示focused suite: **15 / 15 PASS、failure 0、skip 0**
  - 高ラウドネス定常入力のmedium中立化、小さい相対上下、0.55秒hold、track resetを固定。
  - `review-relative-green2-20260713/focused.xcresult`
- iPhone 17 Pro / iOS 27.0 円形波形artifact: **1 / 1 PASS**
  - 新規取得PNGと正本PNGはBMPへデコード後のSHA-256が一致し、画素差なし。
  - `review-relative-visual-ios27-20260713/visual.xcresult`
...
- iPhone 17 Pro Max / iOS 27.0 実機: **署名付きDebug build・ワイヤレスinstall・launch PASS**
  - `review-relative-device-20260713/device.xcresult`
  - `localNetwork`、paired、connected、Developer Mode有効を確認し、`com.codex.yagyoplayer`を上書き起動した。

iOS 27.0 beta Simulatorでは、既存 `PlaybackControllerTests` の同期 `AVAudioSession.setActive(true)` がruntime内で返らず、Simulator完全再起動後も同じ位置で停止した。この12件はiOS 26.5で全件greenを確認し、iOS 27では残る115件を全件実行した。今回の差分は再生backend、Audio Session、解析DSP計算を変更していない。円形波形の相対化は既存15 Hz MainActorメーター上の表示専用O(1)状態で、オーディオ処理スレッドへ入らない。

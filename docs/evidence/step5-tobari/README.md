...
- さらに「現在再生中の曲を新しいBとして選ぶと、Bの音だけが残ってUIはAへ戻る」review経路を、新旧どちらのBがcurrentでも先にpauseする境界へ修正した。Aや無関係な曲は止めない既存契約を維持し、`PlaybackControllerTests`は **22 / 22 PASS**。xcresultは同SSDの`/Volumes/MacBook_Data_Add/CodexDerivedData/YagyoPlayer/pr24-review-fix-20260714/focused-green.xcresult`。
- iPhone 17 Pro / iOS 26.5の最終全回帰は **142 / 142 PASS、failure 0、skip 0**。解析、再生、取込、行列、巻物、3タブと全実描画artifactを含む。xcresultは同SSDの`/Volumes/MacBook_Data_Add/CodexDerivedData/YagyoPlayer/pr24-review-fix-20260714/full-green.xcresult`。

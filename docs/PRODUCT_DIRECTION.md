...
- **現在地(正直に)**: **Phase A(一画面検聴)は完了し`main`へ統合済み**(PR #16、ユーザー本人が実機確認のうえ承認・2026-07-12)。ITU-R BS.1770-4のIntegrated/Max Short-term、Sample Peak、True Peak(4x)、クリップ疑いラン、モノ互換相関を、[Mastering-App](https://github.com/tinypony777/Mastering-App)のDSPコアを補修して移植。Phase Bは現行`main`からのfeature branchで、参照1曲、同時刻A/B、Integrated Loudness差の減衰専用match、基準音量と別の非永続乗数、解除境界まで実装し、全回帰と実機installを完了([証跡](evidence/step5-tobari/README.md))。ユーザー実機操作確認とPR統合は未完了。チェックポイント(C)、共有シート取込(D)は未実装。

...
- 信頼を年数に頼らず、**検証可能な事実**から始める: 通信コードを持たないことの証明(iOS には外向き通信を封じる権限や entitlement が存在しないため「権限がない」は証明にならない — 自前コードと依存ライブラリに通信経路が存在しないことのコード監査と、プロキシ監視下での実測ゼロ通信テストで示す)、App Store プライバシーラベル「データ収集なし」、プライバシーマニフェスト、価格方針の公開。

...
- **日付:** 2026-07-13
- **状態:** ユーザー承認済みの推奨案を文章化。実装開始前の最終レビュー対象
...
目指すものは、夜の濃紺を舞台に、生成り紙へ朱・柿・青緑・焦茶を刷った大正期の商標札やレコードレーベルを重ねたような画面である。添付された三枚の参考画像は、配色、罫線、札形、文字組み、限定色の正本として使う。画像内の英字サンプル、花、人物、鶴、透かしは製品要件として模写しない。
...
アプリ全体を明るい紙色へ反転せず、夜の濃紺を背景として維持する。その上へ、生成りの罫、朱の印、柿色の操作、青緑の狐火を限定的に置く。大面積のグラデーション、半透明ガラス、ネオンglowは撤去し、フラットな版ずれを思わせる面と線で構成する。

`YagyoBackdrop` は通常時を単色 `night`、丑三つ時を既存 `ushiSumi` に近い赤紫の単色面とし、星と流れる靄を静的な小紋・短い霞罫へ置き換える。背景pattern自体は動かさず、通常／丑三つ時の切替だけを既存時間でcross-fadeする。
...
| Token | Hex | 用途 |
|---|---:|---|
| `night` | `#101722` | 全画面の濃紺地 |
| `nightRaised` | `#172331` | 夜色パネル |
| `paper` | `#E8D7B5` | 生成り札、主要罫 |
| `paperMuted` | `#B9A98C` | 副次罫、補助文字 |
| `ink` | `#35241E` | 紙面上の文字と焦茶罫 |
| `vermillion` | `#C94F35` | 朱の印、再生済み、警告 |
| `persimmon` | `#D77A3D` | 主操作、現在位置 |
| `teal` | `#1B6666` | 紙色上の狐火の帳、分析状態 |
| `tealLight` | `#82C8C0` | 夜色上の分析文字と状態罫 |
...
`night`、`paper`、`tobari` の三toneを持つ表層用modifier。単色面、二重罫、任意の小札を描く。`night` は `nightRaised` 面＋外側 `paper`＋内側 `brass`、`paper` は `paper` 面＋外側 `ink`＋内側 `vermillion`、`tobari` は `night` 面＋外側 `paper`＋内側 `tealLight` とする。iOS 26 `glassEffect` と `ultraThinMaterial` は使用しない。既存 `ritualPanel` の呼び出しは、画面ごとに意図が分かるこのmodifierへ移す。
...
1. 夜色の円盤。
2. 生成りの細い同心円を2本。レコード溝兼、丸紋の輪郭。
3. 48本の放射罫。12本ごとに太い主目盛、4本ごとに中目盛を置く。
4. `progress` を示す朱の円弧。12時位置から時計回りに描く。
5. 現在位置を示す柿色の小さな丸紋。円弧末端へ固定する。
6. 中央に生成りの小札を置き、停止時は「止」、利用不可時は「—」、lowは「静」、mediumは「響」、highは「烈」と一文字で示す。VoiceOver情報は既存の親要素に残し、この文字は装飾表示として扱う。
...
- 取込ボタンを角印形へ変更。
- 夜行絵巻はピクセル描画を維持し、外周だけを夜色＋生成り二重罫へ変更。
...
- TrackRowは紙札を積むのではなく、夜色の行へ生成り罫と小さな朱印を置く。長いライブラリで視覚ノイズを増やさない。
...
- タブバーは濃紺地、生成りの非選択、柿色の選択。カスタムタブバーへ置き換えない。
- ミニ灯りは一行の横札とし、現在曲に朱の丸印、右端に柿色の再生印を置く。

### 8.5 狐火の帳

- 静的背景と一画面検聴を維持する。
- 外周を生成りと青緑の二重罫、見出しを生成り札、指標領域を濃紺の計器盤にする。
- metricは左に名称、右にmonospaced値という現在の読み順を保つ。
- 提案は朱または柿の小見出し、根拠は青緑の単罫で区切る。
...
- 紙色上の文字は`ink`、夜色上の文字は`paper`を原則とし、通常文字4.5:1、大文字見出し3:1以上を確認する。
- 指定hexの基準コントラストは `paper/night = 12.69:1`、`paperMuted/night = 7.81:1`、`ink/paper = 10.42:1`、`persimmon/night = 5.77:1`、`teal/paper = 4.72:1`、`tealLight/night = 9.40:1`。不透明度を下げる本文表現は禁止し、装飾罫だけにopacityを許す。
...
コードがgreenでも、UIはユーザー本人の見た目承認まで完成扱いにしない。最初の実装では最低限、次の比較を同じ端末寸法で提示する。
